...
# --- Exemple de référence few-shot (Patch 1)
REF_EXAMPLE = """
...
# =========================================================
#                  LECTURE CLÉ API & SIDEBAR
# =========================================================
api_key_from_file = None
if os.path.exists("config.txt"):
    try:
        with open("config.txt") as f:
            api_key_from_file = f.read().strip() or None
    except Exception:
        api_key_from_file = None

with st.sidebar:
    st.header("⚙️ Paramètres")
    st.write("Clé OpenAI (priorité à `config.txt` si présent)")
    api_key_input = st.text_input("Saisir une clé (optionnel)", type="password", placeholder="sk-...")
    api_key = api_key_from_file or (api_key_input or None)
...
def get_openai_client():
    if not api_key:
        st.warning("⚠️ Aucune clé API détectée. Ajoute-la dans `config.txt` ou renseigne-la dans la sidebar.")
        return None
    try:
...
if mode == "Champs guidés":
    ...
    if submitted:
        # — Preamble de pertinence (Patch 1 + Patch 2)
        preamble_parts = [
